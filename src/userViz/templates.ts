...
export default function MyR3FViz({ audioData, beat, energy, currentTime }: VizProps) {
  const audioRef = useRef<AudioSnapshot>({ audioData, beat, energy, currentTime })
  useEffect(() => {
    audioRef.current = { audioData, beat, energy, currentTime }
  })
...
export default function MyShaderViz({ audioData, beat, energy, currentTime }: VizProps) {
  const canvasRef = useRef<HTMLCanvasElement>(null)

  // RAF читает аудио только из ref — иначе кадры начнут отставать на одно обновление
  const audioRef = useRef({ audioData, beat, energy, currentTime })
  useEffect(() => {
    audioRef.current = { audioData, beat, energy, currentTime }
  })
