...
export function drawBeatGamePreview(ctx: Ctx, w: number, h: number, t: number, isHovered: boolean) {
  fillBg(ctx, w, h)
  const amp = fakeAmplitude(t, isHovered)
  const cols = 10
  const rows = 6
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      const seed = r * 13 + c * 7
      const beat = Math.sin(t * 0.08 + seed * 0.25) > 0.6 - amp * 0.3
      ctx.fillStyle = strokeColor(beat ? 0.7 + amp * 0.25 : 0.06)
