...
export default function Transport({ hasTrack }: TransportProps) {
  ...
  return (
    <div className="flex items-center justify-center" style={{ gap: 18 }}>
      <SideBtn
        disabled={!canStep || preparing}
        title="Предыдущий трек"
        onClick={() => void stepTo('prev')}
      >
        <SkipBack size={16} />
      </SideBtn>
      ...
      <button
        type="button"
        onClick={togglePlay}
        disabled={!hasTrack || preparing}
        title={isPlaying ? 'Пауза' : 'Воспроизвести'}
        style={{
          width: 44,
          height: 44,
          borderRadius: '50%',
          background: hasTrack && !preparing ? 'var(--fg)' : 'var(--bg-elev)',
          color: hasTrack && !preparing ? 'var(--bg)' : 'var(--fg-mute)',
          border: 'none',
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'center',
          cursor: hasTrack && !preparing ? 'pointer' : 'not-allowed',
          opacity: hasTrack && !preparing ? 1 : 0.5,
          transition: 'transform 0.15s',
        }}
        onMouseEnter={(e) => {
          if (hasTrack && !preparing) e.currentTarget.style.transform = 'scale(1.05)'
        }}
        onMouseLeave={(e) => {
