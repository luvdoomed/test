      ...
      >
        ...
        >
          <section>
            ...
            <p>
              Плеер передаёт в виз пропсы аудио и синхронного текста:
            </p>
            <ul style={{ paddingLeft: 18, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <li><Code>lrcLines: {'{ time, text }[]'}</Code> — строки LRC из LRCLIB или файла .lrc.</li>
              <li><Code>activeLineIndex: number</Code> — индекс текущей строки (−1, если текста нет).</li>
              <li><Code>activeLineText: string</Code> — текст активной строки (удобно для canvas/WebGL).</li>
              <li><Code>isPlaying: boolean</Code> — идёт ли воспроизведение.</li>
            </ul>
          </section>

          <section>
            <SectionTitle>Караоке и текст</SectionTitle>
            <p style={{ marginBottom: 6 }}>
              Для всех виз, включая свои, в плеере доступны те же функции, что и для встроенных:
            </p>
            <ul style={{ paddingLeft: 18, marginTop: 4, marginBottom: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
              <li>
                Кнопка <strong>«Караоке»</strong> — синхронный текст поверх визуализатора (не нужно рисовать его в коде).
              </li>
              <li>
                Иконка поиска рядом с названием трека — подбор текста в LRCLIB, ручная правка метаданных, загрузка .lrc.
              </li>
              <li>
                Пропы <Code>lrcLines</Code> / <Code>activeLineText</Code> — если хотите рисовать текст сами (в canvas, WebGL, R3F).
              </li>
            </ul>
            <p>
              Несколько первых четырёх пропов в коде можно игнорировать — старые визы без текста продолжат работать.
              Новые поля всегда приходят от плеера, даже если вы их не деструктурируете.
            </p>
          </section>
...
{EXAMPLE_CODE}
          ...
          <section>
            ...
            <ul style={{ paddingLeft: 18, marginTop: 6, display: 'flex', flexDirection: 'column', gap: 4 }}>
              ...
              <li>
                Для караоке в canvas: читай <Code>activeLineText</Code> из ref на каждом кадре — как в шаблоне Canvas 2D.
              </li>
            </ul>
          </section>
